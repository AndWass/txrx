...
use txrx::traits::{Sender, Receiver, SenderFor, Connection, Scheduler};
use txrx::SenderExt;
...
fn main() {
    ...
    let res = txrx::factories::just(()).and_then(|_| {
        println!("Sleeping for 3 seconds on {:?}", std::thread::current().id());
